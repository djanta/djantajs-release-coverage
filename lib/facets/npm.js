...
module.exports = {
  ...
  build: (repo, directory, callback) => {
    ...
    if (!_.isNil(repo.tag) || true === repo.tag) {
      ...
      let project = utils.isDirectory(repo.project) ? repo.project : path.join(directory, repo.name);
      ...
      if (!_.isNil(pkg) && utils.isFile(pkg)) {
        ...
        let bumper = repo.version && (_.isNil(semver.valid(repo.version)) || !!~utils.IDS.allow.indexOf(repo.version)) ?
            util.format('npm version %s', repo.version) : 'npm version patch';
