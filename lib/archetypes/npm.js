    ...
    if(_.isUndefined(config.tag) || true === config.tag) {
      ...
      if (utils.isFile(_package)) {
        ...
        let _releaselogGenerate = () => {
          console.warn('Changelog auto generation has been desactivated!');
          /*if (config.changelog || /(yes|Yes|y|Y)$/.test(config.changelog)) {
            shell.exec('npm i -g git-release-notes', options);
        ...
        let flows = {
          ...
          add: ['git add', '--verbose', '.'],
          commitChangedVersion: ['git commit', '-a', '-m', '"' + util.format(config.messages.commit, label) + '"'],
        ...
        flows.releasingTagMaster = ['git tag', '-m', '"' + util.format(config.messages.tag, tag) + '"', '-a', tag];
        ...
        try {
          Object.keys(flows).forEach(name => {
            ...
            else { shell.exec (flows[name].join(' '), options); }
          });
        }
        catch (err) {
          console.error(err);
